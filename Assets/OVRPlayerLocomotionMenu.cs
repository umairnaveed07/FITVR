...
public class OVRPlayerLocomotionMenu : MonoBehaviour {
  ...
  private void Awake() {
    playerControllerWithHandPoses =
        GetComponent<OVRPlayerControllerWithHandPoses>();

    onLocomotionAction.WhenSelect.AddListener(() => {
      locomotionOn = !locomotionOn;
      var locomotionMenuOption =
          onLocomotionAction.GetComponentInChildren<TextMeshPro>();
      ...
      Logger.Instance.LogInfo(
          $"Locomotion state changed to: {locomotionMenuState}");
    });
  }
  ...
  private void Update() {
    locomotionMenu.transform.position =
        targetHand.transform.position + offsetFromHand;
    locomotionMenu.transform.rotation = Quaternion.LookRotation(
        locomotionMenu.transform.position -
            cameraRig.centerEyeAnchor.transform.position,
        Vector3.up);
  }
}
